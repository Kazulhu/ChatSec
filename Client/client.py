...
#
#         pixmap = QPixmap()
#         pixmap.loadFromData(QByteArray(qr_image.getvalue()))
...
#
#     # Méthode dans la classe ChatWindow pour mettre à jour la liste des utilisateurs connectés
#     def update_users_list(self, message):
#         users_message = message[len("USER_LIST:"):]
#         self.users_list.clear()
#         users = users_message.split(',')
...
from PyQt5.QtWidgets import (
    ...
    QTextEdit, QListWidget, QMenu, QAction, QMessageBox, QStackedWidget, QFileDialog, QTextBrowser
)
from PyQt5.QtCore import pyqtSignal, Qt, QPoint, QByteArray
from PyQt5.QtGui import QPixmap

# Importons de la classe TextToSpeech
...
# Fonction pour hasher les mots de passe avec Argon2, sel et poivre
def hash_password(password, salt=None):
    if salt is None:
        salt = secrets.token_hex(32)  # Générer un sel aléatoire
    pepper = "1d87d7d452d7af0b1c2674adfbc3a54ed46d85656ace1bfd9ac81b18b794ae50"
    hashed_password = pyargon2.hash(password + pepper, salt)
    return hashed_password, salt
...
class ClientApp(QStackedWidget):
    def __init__(self):
        ...
        # Demander à l'utilisateur s'il veut activer la synthèse vocale
        self.ask_text_to_speech()
    ...
    def ask_text_to_speech(self):
        # Dialogue pour demander à l'utilisateur s'il veut la synthèse vocale
        reply = QMessageBox.question(self, 'Text to Speech', 'Do you want to enable Text to Speech for incoming messages?',
...
class LoginWindow(QWidget):
    ...
    def launch_chat(self, username):
        chat_window = ChatWindow(username, self.parent.client_ssl, self.parent.text_to_speech)
        self.parent.addWidget(chat_window)
        self.parent.setCurrentWidget(chat_window)
...
class RegisterWindow(QWidget):
    ...
    def register(self):
        ...
        # Générer un secret base32 pour l'OTP
        secret = pyotp.random_base32()
...
class EnterToSubmitTextEdit(QTextEdit):
    # Définir un nouveau signal qui sera émis lorsque la touche Entrée est pressée
    enterPressed = pyqtSignal()

    def keyPressEvent(self, event):
        # Si la touche Entrée est pressée, émettre le signal enterPressed
        if (event.key() == Qt.Key_Return or event.key() == Qt.Key_Enter) and not event.modifiers() & Qt.ShiftModifier:
            self.enterPressed.emit()
        else:
            # Sinon, appeler la méthode keyPressEvent de la classe parente
            super().keyPressEvent(event)

# Fenêtre principale de chat
class ChatWindow(QWidget):
    def __init__(self, username, client_ssl, text_to_speech):
        super().__init__()
        self.username = username
        self.client_ssl = client_ssl
    ...
    def send_message(self):
        message = self.message_input.toPlainText()
        if message:
            self.client_ssl.send(f"MESSAGE:{self.username}:{message}".encode('utf-8'))
            self.message_input.clear()
        ...
        while True:
            try:
                ...
                if message.startswith("USER_LIST:"):
                    ...
                    sender, msg = message[len("DM:"):].split(':', 1)
                    self.display_message(f'{sender}: {msg}')

                    # Lecture à haute voix du message si activé
                    if self.text_to_speech.is_enabled():
                        self.text_to_speech.speak(msg)

                elif message.startswith("FILE_LINK:"):
                    filename = message.split(":")[1]
                    self.display_message(f'File received: <a href="download:{filename}">{filename}</a>')
                else:
